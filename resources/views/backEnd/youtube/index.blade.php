        ...
        <div class="card">
            <div class="card-header pb-0">
                <div class="d-flex justify-content-between">
                    <h4 class="card-title mg-b-0 mt-2">Youtube</h4>
                    ...
                    <table class="table text-md-nowrap" id="example1">
                        <thead>
                            <tr>
                                ...
                                <th class="wd-15p border-bottom-0">Video title</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{$youtubeData->category_name}}</td>
                                <td>{{$youtubeData->video_title}}</td>
                                @if($youtubeData->video_url == null)
                                                         <td>
                                    </td>
                                                        @else
                                                         <td><a target="blank" href="{{$youtubeData->video_url}}"
                                ...
                                <td>{{ date('F d,Y', strtotime($youtubeData->created_at)) }}</td>
                                
                                
                                <td><a href="{{route('youtube.edit', $youtubeData->id)}}" class="btn ripple btn-primary text-white btn-icon"><i class="fe fe-edit"></i></a></td>
                                <td><form action="{{ route('youtube.destroy', $youtubeData->id) }}" method="POST">
