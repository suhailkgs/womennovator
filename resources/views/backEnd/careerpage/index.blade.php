        ...
        <div class="card">
            <div class="card-header pb-0">
                <div class="d-flex justify-content-between">
                    <h4 class="card-title mg-b-0 mt-2">career </h4>
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table text-md-nowrap" id="example1">
                        <thead>
                            <tr>
                                <th class="wd-15p border-bottom-0">  Tittle</th>
                        ...
                        <tbody>
                            @foreach($careerpageDatas as $careerpageData)
                            <tr>
                                <td>{{$careerpageData->title ??''}}</td>
                                 <td>{{$careerpageData->exp ??''}}</td>
                                <td>
                                    {{$careerpageData->location}}
                                </td>
                               
                                <td>@if($careerpageData->status==1)
                                    <span class="badge badge-success-gradient">Active</span>
                                    @else
                                    <span class="badge badge-danger-gradient">Inactive</span>
                                    @endif</td>
                                       <td>
                                    {{ date('d-m-Y', strtotime($careerpageData->created_at))}}
                                </td>
                                <td><a href="{{route('careerpage.edit', $careerpageData->id)}}" class="btn ripple btn-primary text-white btn-icon"><i class="fe fe-edit"></i></a></td>
                                <td><form action="{{ route('careerpage.destroy', $careerpageData->id) }}" method="POST">
